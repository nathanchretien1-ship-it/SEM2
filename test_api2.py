...
if token:
    ...
    req_put = urllib.request.Request(url_patient, data=json.dumps(data_put).encode('utf-8'), headers={'Content-Type': 'application/json', 'Authorization': f'Bearer {token}'}, method='PUT')
    try:
        with urllib.request.urlopen(req_put) as response:
            print("Patient PUT:", response.read().decode('utf-8'))
    except urllib.error.HTTPError as e:
        print("Patient PUT Error:", e.read().decode('utf-8'))
